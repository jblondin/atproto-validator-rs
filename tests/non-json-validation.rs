/*
An example demonstrating validation of an arbitrary object (instead of the `serde_json::Value`-based
validation implemented as part of the "json" feature).
 */

use atproto_validator::{
    Context, DocumentType, Error, Validate, ValidateObject,
    atrium::validate::ValidateProp,
    atrium_lex::{
        LexiconDoc,
        lexicon::{LexObject, LexUserType},
    },
};
use atrium_api::types::string::Datetime;

pub mod lexicons;
use atrium_lex::lexicon::LexString;
use lexicons::record_document_json;

// This definition is taken from the file generated by esquema-codegen (forked from atrium-codegen).
// A `$type` field has been added to match objects normally retrieved from the stream
#[derive(serde::Serialize, serde::Deserialize, Debug, Clone, PartialEq, Eq)]
#[serde(rename_all = "camelCase")]
pub struct RecordData {
    #[serde(rename = "$type")]
    pub r#type: String,
    pub created_at: Datetime,
    pub status: String,
}

impl<'a> Validate<DocumentType<'a>> for RecordData {
    fn validate(&self, doc_type: &DocumentType<'a>, _ctxt: &Context, errs: &mut Vec<Error>) {
        if self.r#type != doc_type.as_str() {
            errs.push(Error::TypeMismatch {
                expected: doc_type.as_str().to_owned(),
                actual: self.r#type.clone(),
            });
        }
    }
}

impl Validate<LexUserType> for RecordData {
    fn validate(&self, def: &LexUserType, ctxt: &Context, errs: &mut Vec<Error>) {
        match def {
            LexUserType::Record(record) => self.validate(record, ctxt, errs),
            _ => errs.push(Error::UnexpectedVariant("record".to_owned())),
        }
    }
}

impl Validate<LexObject> for RecordData {
    fn validate(&self, def: &LexObject, ctxt: &Context, errs: &mut Vec<Error>) {
        if let Some(string_def) = ValidateProp::<LexString>::get_prop(def, "createdAt", errs) {
            self.created_at.as_str().validate(string_def, ctxt, errs);
        }
        if let Some(string_def) = ValidateProp::<LexString>::get_prop(def, "status", errs) {
            self.status.as_str().validate(string_def, ctxt, errs);
        }
    }
}

#[test]
pub fn validate_non_json_object() {
    let nsid = "xyz.statusphere.status".parse().expect("bad nsid");
    let doc = match serde_json::from_value::<LexiconDoc>(record_document_json(&nsid)) {
        Ok(doc) => doc,
        Err(e) => panic!("lexicon deserializtion failed: {e}"),
    };
    let ctxt = Context::from_documents([doc]).expect("failed to build context");

    let object = RecordData {
        r#type: nsid.as_str().to_owned(),
        created_at: Datetime::now(),
        status: "😱".to_owned(),
    };
    let validation_results = object.validate_object(&ctxt, &nsid);
    assert!(validation_results.is_ok());

    let object = RecordData {
        r#type: nsid.as_str().to_owned(),
        created_at: Datetime::now(),
        status: "😱😱".to_owned(),
    };
    let validation_results = object.validate_object(&ctxt, &nsid);
    assert!(validation_results.is_err());
    let error = validation_results.unwrap_err().pop().unwrap();
    let Error::NumGraphemeshOutOfBounds(2, None, Some(1)) = error else {
        panic!("unexpected error {error}!");
    };
}
